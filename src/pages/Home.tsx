...
const Home: React.FC = () => {
    ...
    return (
        <div className="relative pt-48 pb-32 px-4 flex flex-col items-center justify-center text-center">
            ...
            <div className="bg-white p-2 rounded-[35px] shadow-[0_0_50px_rgba(168,85,247,0.3)] mb-10 inline-block">
                <img src="/logo.jpg" alt="GIGILUKO Icon" className="w-24 h-24 rounded-[28px] object-cover" />
            </div>
            ...
            <form onSubmit={handleWaitlist} className="max-w-md w-full bg-white/5 p-12 rounded-[50px] border border-white/10 backdrop-blur-sm">
                <input type="email" placeholder="E-Mail" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full bg-white/5 border border-white/10 px-8 py-4 rounded-2xl mb-4 text-white text-center focus:border-purple-500/50 outline-none" />
                <button type="submit" className="w-full bg-white text-black py-4 rounded-2xl font-black uppercase tracking-widest hover:bg-gray-100 transition-all">{status === "loading" ? "..." : "Join Now"}</button>
            </form>
        </div>
    );
};
