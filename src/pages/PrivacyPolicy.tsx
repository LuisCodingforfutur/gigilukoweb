    ...
    return (
        <LegalPage title="Privacy Policy" lastUpdated="6 May 2026">
            ...
            <section>
                ...
                <p>We use your device's location for two distinct purposes:</p>
                <p><strong>(a) Foreground use ("While Using the App"):</strong> When you open the Discover feature, the map centers on your current position to show nearby venues. This requires standard location permission and is active only while the app is in use.</p>
                <p><strong>(b) Background use ("Always", optional, opt-in only):</strong> If you grant "Always" location permission, the app can detect your physical proximity to Bluetooth Low Energy (BLE) beacons placed in partner venues, enabling automatic check-ins even when the app is closed or in the background. This background processing is triggered exclusively by beacon proximity events — we do not perform continuous background location tracking. Without this permission, automatic check-ins are not available, but manual check-ins remain fully functional.</p>
                <p>In both cases, your precise location coordinates are processed only locally on your device and are never transmitted to our servers or stored. Only the resulting check-in event (venue ID and timestamp) is recorded in your account. The legal basis is your consent (Art. 6(1)(a) GDPR). You can downgrade from "Always" to "While Using the App", or revoke location access entirely, at any time via your device's privacy settings.</p>
